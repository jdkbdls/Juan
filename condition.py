...
class Technical:
    ...
    def price_up(self,m,n): 
        if m >= n:
            c = 0
            for n in range(0,m):
                if self.df['Close'].iloc[-1-n] > self.df['Close'].iloc[-2-n]:
                    c = c + 1
            if c >= n:
                return True
            else:
    ...
    def red_k(self,m,n): 
        if m >= n:
            c = 0
            for n in range(0,m):
                if self.df['Close'].iloc[-1-n] > self.df['Open'].iloc[-1-n]:
                    c = c + 1
            if c >= n:
                return True
            else:
    ...
    def volume_3days_ave_increase(self,n): 
        ...
        if (self.df['Volume'].iloc[-3:].mean()) > (self.df['Volume'].iloc[-12:].mean()*n):
            return True
        else:
            return False
